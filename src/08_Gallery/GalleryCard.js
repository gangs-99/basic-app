...
export default function GalleryCard({item}) {
    ...
    spTags = spTags.map(kw => <span className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2" key={kw}>{kw}</span>);
        ...
        <div className="max-w-sm rounded overflow-hidden shadow-lg">
            <img className="h-48 w-full" 
                src={item.galWebImageUrl.includes('http:') 
                        ...
                        : item.galWebImageUrl} alt={item.galTitle} />
